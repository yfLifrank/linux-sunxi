				    ...
				    struct sg_table *sgt)
{
	...
	ret = mtd_map_buf(mtd, nfc->dev, sgt, buf, nchunks * chunksize, ddir);
	if (ret)
		return ret;
			       ...
			       unsigned int ctrl)
{
	...
	if (dat == NAND_CMD_NONE && (ctrl & NAND_NCE) && !(ctrl & (NAND_CLE | NAND_ALE))) {
		u32 cmd = 0;
		...
		if (sunxi_nand->cmd_cycles--) {
			cmd |= NFC_SEND_CMD2;
			writel(sunxi_nand->cmd[1], nfc->regs + NFC_REG_RCMD_SET);
		}

		sunxi_nand->cmd_cycles = 0;

		if (sunxi_nand->addr_cycles) {
			cmd |= NFC_SEND_ADR | NFC_ADR_NUM(sunxi_nand->addr_cycles);;
			writel(sunxi_nand->addr[0], nfc->regs + NFC_REG_ADDR_LOW);
		}

		if (sunxi_nand->addr_cycles > 4)
			writel(sunxi_nand->addr[1], nfc->regs + NFC_REG_ADDR_HIGH);

		writel(cmd, nfc->regs + NFC_REG_CMD);
		sunxi_nand->addr[0] = sunxi_nand->addr[1] = 0;
					 ...
					 uint8_t *bufpoi, int page)
{
	...
	for (i = data_offs / ecc->size ;
		...
		ret = sunxi_nfc_hw_ecc_read_chunk(mtd, data, data_off,
			...
			&cur_off, &max_bitflips, !i, false, page);
		if (ret < 0)
			return ret;
...
static int sunxi_nfc_probe(struct platform_device *pdev)
{
	...
	if (!IS_ERR(nfc->dmac)) {
		...
		dmac_cfg.src_maxburst = dmac_cfg.dst_maxburst = 4;
		dmaengine_slave_config(nfc->dmac, &dmac_cfg);

		dmac_scfg.para = SUN4I_DDMA_PARA_DST_DATA_BLK_SIZE(128) |
				 SUN4I_DDMA_PARA_DST_WAIT_CYCLES(8) |
				 SUN4I_DDMA_PARA_SRC_DATA_BLK_SIZE(128) |
				 SUN4I_DDMA_PARA_SRC_WAIT_CYCLES(8);
